...
class Codeforces
{
    ...
    public:
        ...
        ll solve()
        {
            ...
            ll m=sz(s);
            string ans="";
            ll first=n-1,second=m-1;
            while(first>=0 && second>=0){
                // yes
                if(s[second]>=a[first]){
                    ans+=char('0'+(s[second]-'0')-(a[first]-'0'));
                    first--;
                    second--;
                }
                else{
                    if(second>=1){
                        ll temp_num=s[second-1]-'0';
                        temp_num*=10;
                        temp_num+=s[second]-'0';
                        char temp='1';
                        temp=char('0'+((temp_num)-(a[first]-'0')));
                        if(temp>='0'&&temp<='9'){
                            ans+=temp;
                        }
                        else{
            ...
            string temp_ans=s.substr(0,second+1);
            reverse(all(temp_ans));
            ans+=temp_ans;
            ll pos=-1;
            reverse(all(ans));
