...
class Atcoder
{
    ...
    public:
        ...
        ll solve()
        {
            ll n,m,k;
            cin>>n>>m>>k;
            vector<ll> applicant_demand(n);
            vector<ll> size_of_appartment(m,0);
            for(ll i=0;i<n;i++){
                cin>>applicant_demand[i];
            }
            for(ll i=0;i<m;i++){
                cin>>size_of_appartment[i];
            }
            ll count=0;
            srt(applicant_demand);
            srt(size_of_appartment);
            ll first=0,second=0;
            while(first<n&&second<m){
                if(abs(applicant_demand[first]-size_of_appartment[second])<=k){
                    count++;
                    first++;
                    second++;
                }
                else if(applicant_demand[first]>size_of_appartment[second]){
                    second++;
                }
                else{
                    first++;
