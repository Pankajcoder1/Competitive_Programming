...
class Atcoder
{
	...
	public:
		...
		ll solve()
		{
			...
			vl v(n);
			forin(v,n);
			vector<vector<ll>> dp(401,vector<ll>(401,0)),prefix_sum(401,vector<ll>(401,0));
			for(ll i=0;i<n;i++){
				dp[i][i]=0;
				prefix_sum[i][i]=v[i];
			}
			for(ll width=2;width<=n;width++){
				for(ll l=0,r=l+width-1;r<n;l++,r++){
					prefix_sum[l][r]=v[l]+prefix_sum[l+1][r];
					dp[l][r]=INF;
					for(ll x=l;x<r;x++){
						dp[l][r]=min(dp[l][r],dp[l][x]+dp[x+1][r]+prefix_sum[l][r]);
					}
