...
class Codeforces
{
	...
	public:
		...
		ll solve()
		{
			...
			for(ll i=1;i<n;i++){
				vl prefix_sum(k1+1,0); 
				prefix_sum[0]=dp[i-1][0];
				for(ll j=1;j<=k1;j++){
					prefix_sum[j]=(prefix_sum[j-1]+dp[i-1][j])%mod1;
				}
				for(ll j=0;j<=k1;j++){
					if(j-(v[i]+1)>=0){
						dp[i][j]=(prefix_sum[j]+mod1-prefix_sum[j-v[i]-1])%mod1;
					}
					else{
						dp[i][j]=prefix_sum[j]%mod1;
					}
				}

			}
			cout<<dp[n-1][k1]%mod1<<endl;
			return 0;
		}
};
