...
public:
    int longestPalindrome(vector<string>& words) {
        map<string,int> m;
        int length_of_palindrome_string=0;
        for(auto x:words){
            string temp=x;
            reverse(all(temp));
            if(m[temp]){
                length_of_palindrome_string+=4;
                m[temp]--;
            }
            else{
                m[x]++;
        ...
        int flag=0;
        for(auto x:m){
            if(x.ss==1&&flag==0&&x.ff[0]==x.ff[1]){
                length_of_palindrome_string+=2;
                flag=1;
            }
        }
        return length_of_palindrome_string;
    }
};
